...
2. Enable the Email provider and new user signups in Supabase Authentication. To create accounts and sign in without email delivery, disable **Confirm Email** in Supabase Authentication settings (email addresses will be unverified). With confirmation enabled, users must confirm signup by email before signing in. Configure the Site URL and allowed redirect URLs to include your production origin and `http://127.0.0.1:5500/` for local testing.
...
The Account dialog offers email/password sign-in and a separate Create account option with a minimum 8-character password, subject to Supabase's password policy. Existing magic-link users need a password set through a trusted Supabase recovery or administrative process first. Creating an account with an existing email does not reset its password or grant access to its data. Passwords are sent directly to Supabase Auth. The server verifies access tokens with Supabase `getUser()` and derives the account ID from the verified user, never from the request body. Database tables and RPC functions are inaccessible to browser roles; only the server service role can access them.
