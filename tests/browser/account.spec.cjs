...
for (const scenario of ["signin", "signup", "duplicate", "invalid"]) {
test(`password authentication: ${scenario}`, async ({ page, context }) => {
  ...
  await context.route("**/api/auth/register", route => {
    credentials = route.request().postDataJSON();
    if (scenario === "duplicate") return route.fulfill({ status: 409, json: { error: "An account already exists for this email. Sign in instead." } });
  ...
  const signup = ["signup", "duplicate"].includes(scenario);
  ...
  if (scenario === "invalid") {
    ...
    await expect(page.locator("#account-message")).toContainText("already exists");
    await expect(page.locator("#sign-in-form")).toBeVisible();
  } else if (scenario === "signup") {
    await expect(page.locator("#account-message")).toContainText("Account created successfully");
    await expect(page.locator("#account-submit")).toHaveText("Sign in");
    await expect(page.locator("#account-toggle")).toHaveText("Create an account");
  ...
  if (!["duplicate", "signup"].includes(scenario)) {
    expect(authPath.pathname).toBe("/auth/v1/token");
    expect(authPath.searchParams.get("grant_type")).toBe("password");
  }
});
}
